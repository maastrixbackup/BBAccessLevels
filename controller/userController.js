...
const userRegister = async (req, res, next) => {
  const { email, password, user_type_id, created_by } = req.body;
  ...
  try {
    ...
    db.query(checkSql, [email], (err, results) => {
      ...
      const insertUser = `insert into tbl_users (user_email,user_password,user_type_id,date_of_registration,created_by) VALUES (?,?,?,?,?)`;

      db.query(
        insertUser,
        [email, password, user_type_id, dateOfRegistration, created_by],
...
const userUpdate = (req, res) => {
  const { id, name, cli, akj, user_type_id, email } = req.body;
  ...
  if (email !== undefined) {
    ...
    db.query(
      "SELECT id FROM tbl_users WHERE user_email = ? AND id != ?",
      [email, id],
      (err, results) => {
        if (err) {
          return res.status(500).json({
  ...
  if (email !== undefined) fieldsToUpdate.user_email = email;

  if (user_type_id !== undefined) {
    const parsedId = Number(user_type_id);
    if (isNaN(parsedId)) {
      return res.status(400).json({
        success: false,
        message: "user_type_id must be a number.",
      });
    }
    fieldsToUpdate.user_type_id = user_type_id;
  }
...
const getUsersAddedByAdmin = async (req, res) => {
  try {
    const sql = `SELECT u.id, u.user_email, u.user_name, u.cli, u.akj, u.date_of_registration, u.user_type_id,ut.name as user_type_name, u.created_by from tbl_users u
    JOIN user_type ut ON u.user_type_id = ut.id
    WHERE u.created_by = "Admin"`;
...
const getRoles = async (req, res) => {
  try {
    const sqlFiltered = `SELECT id,name FROM user_type WHERE id !=1`;
    const sqlAll = `SELECT id,name FROM user_type`;

    db.query(sqlFiltered, (err, filteredResults) => {
      if (err) {
        return res.status(500).json({
          success: false,
